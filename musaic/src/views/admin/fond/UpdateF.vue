...
export default {
  ...
  methods: {
    ...
    async updatefond() {
      if (this.imgModifiee) {
        const storage = getStorage();
        let docRef = ref(storage, "fond/" + this.imageActuelle);
        deleteObject(docRef);
        docRef = ref(storage, "fond/" + this.fond.img);
        await uploadString(docRef, this.imageData, "data_url").then(
          (snapshot) => {
            // console.log("Upload a base64 string", this.fond.img);
          }
        );
      }
      const firestore = getFirestore();
