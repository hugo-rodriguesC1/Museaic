  ...
  >
    ...
    <table class="col-span-full w-full">
      ...
      <tbody>
        ...
        <tr v-for="item in listeItem" :key="item.id">
          <td>
            <img
              :src="item.img"
              :alt="item.nom"
          ...
          <td class="text-center text-white">{{ item.nom }}</td>
          <td>
            <div class="flex flex-row justify-center gap-4">
              <router-link :to="{ name: 'UpdateI', params: { id: item.id } }"
                ><img src="/img/Icon/update.png" alt="modifier" class="w-5"
              /></router-link>
              <router-link :to="{ name: 'DeleteI', params: { id: item.id } }"
    ...
    async getitem() {
      ...
      await onSnapshot(dbitem, (snapshot) => {
        ...
        this.listeItem.forEach(function (it) {
          ...
          const dbitemimg = ref(storage, "item/" + it.img);
          getDownloadURL(dbitemimg)
            .then((url) => {
              it.img = url;
            })
            .catch((error) => {
