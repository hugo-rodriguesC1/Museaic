  ...
  >
    ...
    >
      ...
      <h5
        ...
        "
      >
        Attention, vous êtes sur le point de supprimer cet item, cette action
        est irréversible.
      </h5>
...
export default {
  ...
  methods: {
    ...
    async deleteitem() {
      const firestore = getFirestore();
      await deleteDoc(doc(firestore, "item", this.$route.params.id));
      const storage = getStorage();
      let docRef = ref(storage, "item/" + this.item.img);
      deleteObject(docRef);
